...
for Repeat in range(REPEAT):
	...
	for train_pair_idx,valid_pair_idx in kf.split(np.array(pair_ID_trn),np.loadtxt(trn_prefix+'_labels_pair.txt')):#always split on pair_label in order not to information leaking
		...
		'''if US == 1:
			UnderSample = OneSidedSelection(return_indices=True,random_state=Repeat*10+2, n_jobs = 16)
			train_X, tmpy, train_idx = UnderSample.fit_sample(train_X, is10[train_idx])
	...
	for i, clf_name in enumerate(clf_torun):
		...
		os.system('echo "'+tst_prefix+' '+clf_name+'-'+str(isTrip)+' Overall F1_bin: ' + str(f1_score(TF_bin_tst,1*(Holdout_prob_tst[:,i]>0.5)))+'"')
